...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
	...
	@Override
	public Object invokeForRequest(NativeWebRequest request, ModelAndViewContainer mavContainer, Object...
			providedArgs)
			throws Exception {
		if (getClass().equals(ConcurrentResultHandlerMethod.class)) {
		...
		if (nativeResponse instanceof HttpServletResponse) {
			int status = ((HttpServletResponse) nativeResponse).getStatus();
			if (status >= 400) {
				return super.invokeForRequest(request, mavContainer, providedArgs);
			}
		}
		...
		new Thread() {
			...
			public void run() {
				try {
					Object result;
					try {
						result = ServletInvocableHandlerMethod.super
								.invokeForRequest(request, mavContainer, providedArgs);
					} catch (Exception e) {
